...
class ProductDetailViewModel(
    ...
    override fun handleEvent(event: ProductDetailEvent) {
        when (event) {
            ...
            // Wishlist is only toggled in local UI state for now — wired to
            // real Room-backed persistence once WishlistRepository lands.
            is ProductDetailEvent.WishlistToggled ->
                setState { copy(isWishlisted = !isWishlisted) }
